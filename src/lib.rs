...
pub fn create_problem_pkg(problem_name: &str, difficulty: config::Difficulty) -> anyhow::Result<()> {
  ...
  match difficulty {
    config::Difficulty::Easy => {
      if problems.easy.contains_key(problem_name) {
        return Err(anyhow::anyhow!("problem '{}' already exists!", problem_name))
      }
  ...
  problems.write_to_file("./problems.json")?;

  let dest = std::path::Path::new(&dest_path);
  copy_dir(src, dest)
    .with_context(|| format!("Copying directory failed"))?;
  log::info!("Created {} problem '{}'", format!("{}", difficulty).magenta(), problem_name.magenta());
  Ok(())
}
...
pub fn create_problem_docs(problem_name: &str, difficulty: config::Difficulty) -> anyhow::Result<()> {
  ...
  if docs_path.exists() {
    ...
    let problems: Problems = Problems::extract("./problems.json")?;
